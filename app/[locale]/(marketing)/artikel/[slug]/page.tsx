...
import { getArticleBySlug, getArticles, articles as rawArticles } from "@/lib/data/articles";
...
export default async function ArticleDetailPage({ params }: Props) {
  ...
  const relatedArticles = localizedArticles.filter(a => a.id !== article.id).slice(0, 3);
  ...
  return (
    <>
              ...
              <AnimatedElement variant="fadeUp">
                ...
                <div className="mb-10 flex items-center gap-4 rounded-2xl bg-cream-50 p-6 border border-gray-100">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-medium text-gold-600 uppercase">{article.authorRole}</p>
                  </div>
                </div>
      ...
      {relatedArticles.length > 0 && (
        <SectionWrapper background="cream" className="border-t border-gray-200/50">
          <Container>
            ...
            <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
              {relatedArticles.map((relatedArticle, index) => (
                <AnimatedElement key={relatedArticle.id} variant="fadeUp" delay={index * 0.1}>
