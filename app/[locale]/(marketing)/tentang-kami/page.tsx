...
import { Shield, Target, Eye, Compass, Scale, Users, Building2, MapPin } from "lucide-react";
import { siteConfig } from "@/lib/data/site-config";

export const metadata: Metadata = {
  title: "Tentang Kami",
  description: "Sejarah, visi, misi, dan nilai-nilai inti yang membentuk Firma Hukum sebagai penyedia layanan hukum terpercaya.",
};
...
const values = [
  {
    ...
    description: "Kami menjunjung tinggi standar etika profesi terdalam dalam setiap tindakan dan keputusan hukum yang kami ambil.",
  },
  {
    icon: Target,
    title: "Berorientasi pada Solusi",
    description: "Fokus kami adalah menemukan solusi hukum paling efektif dan efisien yang sejalan dengan tujuan strategis klien.",
  },
  {
    icon: Users,
    title: "Kemitraan Jangka Panjang",
    description: "Kami membangun hubungan berbasis kepercayaan dan transparansi untuk menjadi mitra hukum strategis jangka panjang.",
  },
  {
    icon: Scale,
    title: "Keunggulan Profesional",
    description: "Komitmen tanpa henti terhadap kualitas, ketelitian, dan pembaruan pengetahuan hukum secara berkelanjutan.",
  },
];
...
const timeline = [
  {
    ...
    description: "Didirikan oleh Dr. Andi Pratama dengan fokus awal pada litigasi perdata dan pidana di Jakarta.",
  },
  {
    year: "2013",
    title: "Ekspansi Layanan Korporasi",
    description: "Membuka divisi khusus hukum korporasi dan komersial seiring meningkatnya kepercayaan klien bisnis.",
  },
  {
    year: "2018",
    title: "Pengakuan Industri",
    description: "Masuk dalam daftar 50 firma hukum terbaik di Indonesia untuk kategori Corporate and M&A.",
  },
  {
    year: "2023",
    title: "Era Modern",
    description: "Pindah ke kantor pusat baru dan integrasi teknologi legal tech untuk efisiensi layanan klien.",
  },
];
          ...
          <div className="grid gap-12 lg:grid-cols-2 lg:gap-16 items-center">
            <AnimatedElement variant="fadeRight">
              <h2 className="font-heading text-3xl font-bold text-navy-950 md:text-4xl">
                {dict.aboutPage.historyBadge} <span className="text-gold-600">{dict.aboutPage.historyHighlight}</span>
              </h2>
              <div className="mt-6 space-y-6 text-gray-600 leading-relaxed text-lg">
                <p>
                  {dict.aboutPage.historyP1}
                </p>
                <p>
                  {dict.aboutPage.historyP2}
                </p>
                <p>
                  {dict.aboutPage.historyP3}
                </p>
              </div>
            </AnimatedElement>
            ...
            <AnimatedElement variant="fadeLeft" className="relative">
              <div className="aspect-square overflow-hidden rounded-2xl bg-navy-900 relative">
                ...
                <div className="absolute bottom-8 left-8 right-8 border-l-2 border-gold-500 pl-6 py-2">
                  <p className="font-heading text-2xl font-bold text-white mb-2">"{dict.aboutPage.quote}"</p>
                  <p className="text-sm text-gold-400">{dict.aboutPage.quoteDesc}</p>
                </div>
              </div>
            </AnimatedElement>
          </div>
      ...
      <SectionWrapper background="navy" className="relative overflow-hidden">
        ...
        <Container className="relative z-10">
          <div className="grid gap-8 md:grid-cols-2 lg:gap-12">
            <AnimatedElement variant="fadeUp" className="rounded-2xl border border-navy-700 bg-navy-900/50 p-8 backdrop-blur-sm lg:p-12">
              <Eye className="h-12 w-12 text-gold-500 mb-6" />
              <h2 className="font-heading text-2xl font-bold text-white mb-4">{dict.aboutPage.visionTitle}</h2>
              <p className="text-gray-400 leading-relaxed text-lg">
                {dict.aboutPage.visionDesc}
              </p>
            </AnimatedElement>
            
            <AnimatedElement variant="fadeUp" delay={0.2} className="rounded-2xl border border-navy-700 bg-navy-900/50 p-8 backdrop-blur-sm lg:p-12">
              <Compass className="h-12 w-12 text-gold-500 mb-6" />
              <h2 className="font-heading text-2xl font-bold text-white mb-4">{dict.aboutPage.missionTitle}</h2>
                ...
                <div className={`md:flex items-center justify-between ${index % 2 === 0 ? 'md:flex-row-reverse' : ''}`}>
                  ...
                  <div className="md:w-[45%]">
                    <div className="rounded-xl border border-gray-100 bg-cream-50/50 p-6 shadow-sm">
                      <span className="text-sm font-bold text-gold-600 mb-2 block">{item.year}</span>
                      <h3 className="font-heading text-xl font-bold text-navy-950 mb-2">{item.title}</h3>
            ...
            <AnimatedElement variant="fadeRight">
              ...
              <ul className="mt-8 space-y-6">
                <li className="flex gap-4">
                  ...
                  <div>
                    <h4 className="font-bold text-white">{dict.aboutPage.officeFeatures[0].title}</h4>
                    <p className="mt-1 text-sm text-gray-400">{dict.aboutPage.officeFeatures[0].desc}</p>
                  </div>
                </li>
                <li className="flex gap-4">
                  <div className="mt-1 shrink-0">
                  ...
                  <div>
                    <h4 className="font-bold text-white">{dict.aboutPage.officeFeatures[1].title}</h4>
                    <p className="mt-1 text-sm text-gray-400">{dict.aboutPage.officeFeatures[1].desc}</p>
                  </div>
                </li>
              </ul>
            </AnimatedElement>
