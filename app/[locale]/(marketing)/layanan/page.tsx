                  ...
                  <Card className="h-full border-gray-200 bg-white transition-all duration-300 hover:border-gold-300 hover:shadow-xl hover:shadow-gold-900/5">
                    <CardHeader>
                      ...
                      <CardTitle className="font-heading text-2xl leading-tight text-navy-950 transition-colors group-hover:text-gold-600 min-h-[64px] flex items-start">
                        <span className="line-clamp-2">{dict.servicesPage.areas[area.id as keyof typeof dict.servicesPage.areas]?.title}</span>
                      </CardTitle>
                    </CardHeader>
                    <CardContent className="flex flex-col flex-grow">
                      <CardDescription className="text-base leading-relaxed text-gray-600 mb-6 text-justify min-h-[104px]">
                        <span className="line-clamp-4">{dict.servicesPage.areas[area.id as keyof typeof dict.servicesPage.areas]?.shortDesc}</span>
                      </CardDescription>
                      
                      <div className="space-y-2 mb-8">
                        {area.keyServices.slice(0, 3).map((service, idx) => (
                          <div key={idx} className="flex items-start gap-2 text-sm text-gray-500 text-left">
                            <span className="text-gold-500 font-bold mt-0.5">•</span>
                            <span className="line-clamp-1">{dict.servicesPage.areas[area.id as keyof typeof dict.servicesPage.areas]?.keyServices[idx]}</span>
                          </div>
                        ))}
                      </div>
