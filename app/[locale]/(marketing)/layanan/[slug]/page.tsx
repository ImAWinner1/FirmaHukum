...
export async function generateMetadata({
  ...
  return {
    title:
      dict.servicesPage.areas[area.id as keyof typeof dict.servicesPage.areas]
        ?.title || area.title,
    description:
      dict.servicesPage.areas[area.id as keyof typeof dict.servicesPage.areas]
        ?.shortDesc || area.shortDescription,
  };
}
              ...
              <div className="prose prose-lg prose-gray max-w-none text-justify">
                {String(
                  ...
                  .map((paragraph, index) => (
                    <p
                      key={index}
                      className="text-gray-600 leading-relaxed mb-4 indent-8"
                    >
                      {paragraph}
                    </p>
                  ))}
                  ...
                  {practiceAreas
                    ...
                    .map((otherArea) => (
                      <li key={otherArea.id}>
                        ...
                        >
                          {dict.servicesPage.areas[
                            otherArea.id as keyof typeof dict.servicesPage.areas
                          ]?.title || otherArea.title}
